...
new #[Layout('layouts.guest')] class extends Component
{
    ...
    public function login(): void
    {
        ...
        if($user->role === 'technician'){
            $url = route('dashboard_technician', absolute: false);
        } elseif ($user->role === 'customer') {
            $url = route('beranda', absolute: false);
        } elseif ($user->role === 'admin') {
            $url = route('dashboard_admin', absolute: false);
        } else {
            $url = route('welcome', absolute: false);
        }
...
<div class="w-full">
    ...
    <form wire:submit="login" class="space-y-5">
        ...
        <!-- Social Login -->
        <div class="mb-6">
            <button type="button" class="flex items-center w-full justify-center gap-2 px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-200 group">
